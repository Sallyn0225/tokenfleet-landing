...
#
# 环境变量（均可选，默认值可直接用）：
#   TF_REPO    GitHub 仓库，默认 Sallyn0225/tokenfleet-landing
#   TF_BASE    部署根目录，默认 /opt/tokenfleet
set -euo pipefail

repo="${TF_REPO:-Sallyn0225/tokenfleet-landing}"
base="${TF_BASE:-/opt/tokenfleet}"
...
# "<hash>  tokenfleet-landing-dist.zip"，sha256sum -c 按其中记录的文件名查找。
# curl 失败（-f）即整脚本退出，不会把半截包留着部署。
curl -fsSL -o "$staging/tokenfleet-landing-dist.zip" \
  "https://github.com/$repo/releases/latest/download/tokenfleet-landing-dist.zip"
curl -fsSL -o "$staging/tokenfleet-landing-dist.zip.sha256" \
  "https://github.com/$repo/releases/latest/download/tokenfleet-landing-dist.zip.sha256"

# 校验须在临时区内执行（-c 按相对路径查找）。
# 校验失败 → 输出 FAILED 且退出码非 0 → 走这里显式退出，站点保持旧版本。
( cd "$staging" && sha256sum -c tokenfleet-landing-dist.zip.sha256 ) >/dev/null || {
...
rm -rf "$target"
mkdir -p "$target"
unzip -q "$staging/tokenfleet-landing-dist.zip" -d "$target"
