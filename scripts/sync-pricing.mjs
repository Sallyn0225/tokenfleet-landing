 ...
 * https://tokenfleet.ai/api/pricing and report what changed.
 *
...
const PRICING_URL = 'https://tokenfleet.ai/api/pricing';
const LOGIN_URL = 'https://tokenfleet.ai/api/user/login';
 ...
 * overseas branch has no standalone consistency check (main's
 * `check:catalog` depends on main-only data files); the per-model tables
 * covered here are the ones listed in `docs/model-catalog-maintenance.md`.
 */
async function loadManualKeySets() {
  ...
  // Overseas branch scope: no `model-meta.ts` (main-only); the four-locale
  // i18n aggregate mirrors `i18n.{zh,en,ja,ko}.featured.blurbs`.
  await add(
  ...
  for (const locale of ['zh', 'en', 'ja', 'ko']) {
    await add(
      `src/i18n.ts (${locale} featured.blurbs)`,
      '../src/i18n.ts',
      (m) => m.i18n[locale].featured.blurbs
    );
  }
...
/**
 * Inlined from `src/data/pricing.ts` (same name, same semantics) so this
 * script stays loadable: that module imports `pricing-api.json`, which Node
 * rejects without an import attribute. Keep the two implementations in sync —
 * the main branch keeps its copy in the import-free `catalog-overrides.ts`;
 * this branch has no such module, so the function lives here instead.
 */
const NO_COLOR_VARIANT = new Set(['openai', 'moonshot', 'anthropic']);

function iconSlugFromField(field) {
 ...
 * the colour slug comes from the inlined `iconSlugFromField` above.
 *
...
async function buildSummary(previous, next, snapshotChanged) {
  ...
  if (removed.length) {
    ...
    out.push('');
    out.push(
      '> 上述人工数据表为海外分支全部维护点（见 `docs/model-catalog-maintenance.md` §2）；其余覆盖表不存在于本分支，无需残留检查。'
    );
    out.push('');
  }
  ...
  out.push(
    '新增模型的人工字段缺口（展示名 / 类型归类 / TPM·RPM / 图标）见 `docs/model-catalog-maintenance.md` §2 —— 空值是合法设计意图，不阻断上线。'
  );
