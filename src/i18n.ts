...
export const i18n = {
  zh: {
    ...
    seo: {
      ...
      modelsDescription: (total: number, vendorCount: number) =>
        `覆盖 ${vendorCount} 家厂商的 ${total} 个生产级 AI 模型。按模型类型筛选，查看每个模型的 TPM / RPM 限速，一个 endpoint 调用所有。`,
    },
    nav: {
      aria: '主导航',
      links: {
    ...
    models: {
      ...
      body: (vendorCount: number) =>
        `覆盖 ${vendorCount} 家厂商。按模型类型筛选，查看每个模型的 TPM / RPM 限速。`,
  ...
  en: {
    ...
    seo: {
      ...
      modelsDescription: (total: number, vendorCount: number) =>
        `Browse ${total} production AI models across ${vendorCount} vendors. Filter by model type and check TPM / RPM rate limits for each model.`,
    },
    nav: {
    ...
    models: {
      ...
      title: (total: number) =>
        `${total} production models —\nnames, IDs & rate limits.`,
      body: (vendorCount: number) =>
        `Covering ${vendorCount} vendors. Filter by model type and check TPM / RPM rate limits for each model.`,
